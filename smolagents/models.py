...
class OpenAIServerModel(ApiModel):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # to_parse = response.choices[0].message.model_dump(include={"role", "content", "tool_calls"})
        # if "parameters" in to_parse['tool_calls'][0]['function']:
        #     print("======redundant parameters. deleted=========")
        #     del to_parse['tool_calls'][0]['function']["parameters"]
        first_message = ChatMessage.from_dict(
            response.choices[0].message.model_dump(include={"role", "content", "tool_calls"})
        )
        first_message.raw = response
